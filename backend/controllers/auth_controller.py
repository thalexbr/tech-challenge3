...
from backend.config.security import create_access_token, get_password_hash, verify_password,Token, UserData
...
def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    **Exemplo de Resposta:**
    ...
    # if not verify_password(form_data.password, 'senha'):
    ...
    # Validando a senha do usuário
    if not (form_data.password == 'senha'):
    # if not verify_password(form_data.password, user['senha']):
        raise HTTPException(
            status_code=401,
            detail='Incorrect email or password',
    ...
    access_token = create_access_token(
        data={"sub": user, "role": 'admin'}, expires_delta=access_token_expires
